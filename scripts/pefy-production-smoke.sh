...
log "Checking Project Gallery surface"
GALLERY_STATUS="$(curl --silent --show-error --location --connect-timeout 5 --max-time 20 \
  --output "$EVIDENCE_DIR/project-gallery.html" --write-out '%{http_code}' \
...
log "Proving unauthenticated access is denied"
DENIED_STATUS="$(curl --silent --show-error --connect-timeout 5 --max-time 15 \
  --output "$EVIDENCE_DIR/unauthenticated-bootstrap.json" --write-out '%{http_code}' \
...
if [[ -n "$AUTH_TOKEN" ]]; then
  log "Checking authenticated bootstrap with supplied production token (token is never written to evidence)"
  AUTH_STATUS="$(curl --silent --show-error --connect-timeout 5 --max-time 15 \
    --output "$EVIDENCE_DIR/authenticated-bootstrap.json" --write-out '%{http_code}' \
...
python3 - "$EVIDENCE_DIR" "$BACKEND_URL" "$FRONTEND_URL" "${GALLERY_STATUS}" "${DENIED_STATUS}" "${AUTH_TOKEN:+present}" <<'PY'
...
summary = {
    ...
    "checks": {
        ...
        "authenticated_bootstrap": "passed" if sys.argv[6] == "present" else "not_run",
    },
...
log "Smoke checks complete. Evidence written to $EVIDENCE_DIR"
